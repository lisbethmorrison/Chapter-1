...
## read data
## butterflies
ukbms_strat <- read.csv( "../Results/Model_outputs/average_spec_ukbms.csv", header=TRUE)
ukbms_mob <- read.csv("../Results/Model_outputs/average_mob_ukbms.csv", header=TRUE)
ukbms_common <- read.csv("../Results/Model_outputs/average_abund_ukbms.csv", header=TRUE)
ukbms_fixed <- read.csv("../Results/Model_outputs/fixed_effect_results_ukbms.csv", header=TRUE)
## cbc birds
cbc_strat <- read.csv( "../Results/Model_outputs/average_spec_cbc.csv", header=TRUE)
cbc_mob <- read.csv("../Results/Model_outputs/average_mob_cbc.csv", header=TRUE)
cbc_common <- read.csv("../Results/Model_outputs/average_abund_cbc.csv", header=TRUE)
cbc_fixed <- read.csv("../Results/Model_outputs/fixed_effect_results_cbc.csv", header=TRUE)
## bbs birds
bbs_strat <- read.csv( "../Results/Model_outputs/average_spec_bbs.csv", header=TRUE)
bbs_mob <- read.csv("../Results/Model_outputs/average_mob_bbs.csv", header=TRUE)
bbs_common <- read.csv("../Results/Model_outputs/average_abund_bbs.csv", header=TRUE)
bbs_fixed <- read.csv("../Results/Model_outputs/fixed_effect_results_bbs.csv", header=TRUE)
...
ukbms_fixed <- ukbms_fixed[-c(1,5:31),]
cbc_strat <- cbc_strat[-c(1:5),] 
cbc_mob <- cbc_mob[-c(1:5),] 
cbc_common <- cbc_common[-c(1:5),] 
cbc_fixed <- cbc_fixed[-c(1,5:15),]
...
lookup <- c("specialismwider.countryside"="Specialism", "specialismgeneralist"="Specialism", "specialismgeneralist"="Specialism",
...
figure3 <- ggplot(data=average_synchrony,aes(x=vars,y=Estimate, group=Scheme),position=position_dodge(width=0.5)) +
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
ggsave(filename="../Graphs/FINAL/Figure3.pdf", plot=figure3, width=173, height=100, units="mm")
