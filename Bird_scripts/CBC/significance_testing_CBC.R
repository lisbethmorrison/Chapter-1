...
## 4 species skipped (347, 377, 450, 456) 25 species total
...
length(unique(pair_attr_comp$spp)) ## 25
spp_list <- unique(F_result_final$j) ## only run this on the species that work (24)
...
print(paste0("TOTAL RUN TIME: ", para_run_time)) ## 42mins for 999 runs
#### end
...
nrow(final_results_cbc[final_results_cbc$change=="No change",]) ## 19 species unchanged
...
final_results_cbc2 <- data.frame(change=unique(final_results_cbc$change), no_species=c(19,4,2), total_species=25)
final_results_cbc2$percentage <- (final_results_cbc2$no_species / final_results_cbc2$total_species)*100

## save file
write.csv(final_results_cbc2, file="../Results/Bird_results/cbc_overall_trends.csv", row.names=FALSE)
