...
pair_attr <- read.csv(file = "../Data/Bird_sync_data/pop_climate_synchrony_BBS.csv", header=TRUE) # read in pair_attr file 
...
all_spp_model_family <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                               mid.year + family + (1|pair.id) + (1|spp), data = pair_attr)
...
all_spp_model_genus <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                              mid.year + genus + (1|pair.id) + (1|spp), data = pair_attr)
...
all_spp_model <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                        mid.year + (1|pair.id) + (1|spp), data = pair_attr)
...
all_spp_model <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                        mid.year + (1|pair.id) + (1|spp)-1, data = pair_attr)
...
climate_model1 <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                        mid.year + (1|pair.id) + (1|spp), data = pair_attr)
...
for (i in unique(pair_attr$spp)){
  ...
  species_model <- lmer(lag0 ~ mean_northing + distance + renk_hab_sim + winter_rain + autumn_rain + spring_rain + 
                          mid.year + (1|pair.id)-1, data = pair_attr[pair_attr$spp==i,])
